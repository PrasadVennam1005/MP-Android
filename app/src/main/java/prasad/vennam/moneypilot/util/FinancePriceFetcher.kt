...
object FinancePriceFetcher {
    ...
    data class YahooChartResponse(val chart: YahooChartData)
    data class YahooChartData(val result: List<YahooChartResult>?)
    data class YahooChartResult(val meta: YahooChartMeta)
    data class YahooChartMeta(val regularMarketPrice: Double)

    data class AmfiResponse(val meta: AmfiMeta, val data: List<AmfiData>?)
    data class AmfiMeta(val scheme_name: String)
    data class AmfiData(val date: String, val nav: String)

    // ─── Historical price response models ─────────────────────────────────────
    data class YahooHistChart(val chart: YahooHistChartData)
    data class YahooHistChartData(val result: List<YahooHistResult>?)
    data class YahooHistResult(
        val timestamp: List<Long>?,
        val indicators: YahooHistIndicators?
    )
    data class YahooHistIndicators(val quote: List<YahooOhlcv>?)
    data class YahooOhlcv(val close: List<Double?>?)
    ...
    // ─── AMFI search model ────────────────────────────────────────────────────
    data class AmfiSearchItem(val schemeCode: Int, val schemeName: String)

    // ─────────────────────────────────────────────────────────────────────────
    // Live price: Yahoo Finance (Stocks, Crypto, Gold, ETFs)
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun fetchYahooPrice(symbol: String): Double? = withContext(Dispatchers.IO) {
        runCatching {
            ...
            client.newCall(req).execute().use { resp ->
                ...
                moshi.adapter(YahooChartResponse::class.java)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun fetchAmfiNav(schemeCode: String): Double? = withContext(Dispatchers.IO) {
        runCatching {
            val req = Request.Builder()
            ...
            client.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) return@withContext null
                moshi.adapter(AmfiResponse::class.java)
                    .fromJson(resp.body.string())
                    ?.data?.firstOrNull()?.nav?.toDoubleOrNull()
            }
        }.getOrNull()
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun fetchPriceOnDate(symbol: String, dateMs: Long): Double? = withContext(Dispatchers.IO) {
        runCatching {
            ...
            client.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) return@withContext null
                val histChart = moshi.adapter(YahooHistChart::class.java)
                    .fromJson(resp.body.string())
                val result = histChart?.chart?.result?.firstOrNull()
    ...
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun fetchNavOnDate(schemeCode: String, dateMs: Long): Double? = withContext(Dispatchers.IO) {
        runCatching {
            val req = Request.Builder()
                .url("https://api.mfapi.in/mf/$schemeCode")
                .header("User-Agent", "Mozilla/5.0")
            ...
            client.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) return@withContext null
                val allData = moshi.adapter(AmfiResponse::class.java)
                    .fromJson(resp.body.string())?.data
                    ?: return@withContext null
    ...
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun searchStockSymbols(query: String): List<SymbolResult> = withContext(Dispatchers.IO) {
        ...
        runCatching {
            ...
            client.newCall(req).execute().use { resp ->
                ...
                moshi.adapter(YahooSearchResponse::class.java)
                    .fromJson(resp.body.string())
                    ?.quotes
                    ?.filter {
                        it.symbol != null &&
    ...
    // ─────────────────────────────────────────────────────────────────────────
    suspend fun searchMutualFunds(query: String): List<SymbolResult> = withContext(Dispatchers.IO) {
        if (query.isBlank()) return@withContext emptyList()
        runCatching {
            val req = Request.Builder()
            ...
            client.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) return@withContext emptyList()
                val listType = moshi.adapter<List<AmfiSearchItem>>(
                    com.squareup.moshi.Types.newParameterizedType(
                        List::class.java, AmfiSearchItem::class.java
                    )
                )
                listType.fromJson(resp.body.string())
                    ?.take(8)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    fun goldSuggestions(): List<SymbolResult> = listOf(
