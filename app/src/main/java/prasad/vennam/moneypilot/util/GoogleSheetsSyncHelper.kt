...
object GoogleSheetsSyncHelper {
    ...
    private fun downloadSpreadsheetData(token: String, spreadsheetId: String): List<ValueRange>? {
        ...
        for (vr in valueRanges) {
            ...
            if (range.contains("Categories")) {
                for (row in values) {
                    ...
                    val color = row.getOrNull(3)?.toString()?.toDoubleOrNull()?.toLong() ?: 0L
                ...
                for (row in values) {
                    val idStr = row.getOrNull(0)?.toString().orEmpty()
                    val id = idStr.toDoubleOrNull()?.toLong() ?: 0L
                    val categoryId = row.getOrNull(1)?.toString()?.toDoubleOrNull()?.toLong() ?: continue
                    ...
                    repository.insertBudget(Budget(id = id, categoryId = categoryId, amount = (amount * 100).toLong(), period = period, currencyCode = currencyCode))
                }
                ...
                for (row in values) {
                    ...
                    if (name.isNotEmpty()) {
                        repository.insertInvestment(Investment(id = id, name = name, type = type, investedAmount = (investedAmount * 100).toLong(), currentValue = (currentValue * 100).toLong(), currencyCode = currencyCode))
                    }
                }
                ...
                for (row in values) {
                    ...
                    val categoryId = row.getOrNull(3)?.toString()?.toDoubleOrNull()?.toLong()
                    ...
                    repository.insertTransaction(Transaction(id = id, amount = (amount * 100).toLong(), timestamp = timestamp, categoryId = categoryId, paymentMode = paymentMode, note = note, type = type, currencyCode = currencyCode))
                }
            }
        }
    }
    ...
    private suspend fun uploadLocalDataToSpreadsheet(
        ...
        val tRows = mutableListOf<List<Any>>(listOf("ID", "Date", "Type", "Category ID", "Payment Mode", "Amount", "Note", "Currency Code"))
        for (t in transactions) {
            val dateStr = dateFormat.format(Date(t.timestamp))
            tRows.add(listOf(t.id, dateStr, t.type.name, t.categoryId ?: "", t.paymentMode, t.amount / 100.0, t.note, t.currencyCode))
        }
        ...
        val iRows = mutableListOf<List<Any>>(listOf("ID", "Name", "Type", "Invested Amount", "Current Value", "Currency Code"))
        for (i in investments) iRows.add(listOf(i.id, i.name, i.type, i.investedAmount / 100.0, i.currentValue / 100.0, i.currencyCode))
