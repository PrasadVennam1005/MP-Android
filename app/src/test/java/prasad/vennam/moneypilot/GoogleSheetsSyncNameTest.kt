...
class GoogleSheetsSyncNameTest {
    ...
    @Test
    fun testMergeCloudDataIntoLocal_insertsNewerCloudCategory() = runBlocking {
        ...
        GoogleSheetsSyncHelper.mergeCloudDataIntoLocal(repository, mockUserPrefs, cloudValues)
    ...
    @Test
    fun testMergeCloudDataIntoLocal_ignoresOlderCloudCategory() = runBlocking {
        val (repository, fakes) = createMockRepository()
        val mockUserPrefs = mock(UserPreferences::class.java)
    ...
    @Test
    fun testMergeCloudDataIntoLocal_insertsNewerCloudTransaction() = runBlocking {
        val (repository, fakes) = createMockRepository()
        val mockUserPrefs = mock(UserPreferences::class.java)
    ...
    @Test
    fun testMergeCloudDataIntoLocal_ignoresOlderCloudTransaction() = runBlocking {
        val (repository, fakes) = createMockRepository()
        val mockUserPrefs = mock(UserPreferences::class.java)
    ...
    private class FakeBudgetDao : BudgetDao {
        ...
        override suspend fun getBudget(categoryId: Long, period: String): Budget? = budgets.find { it.categoryId == categoryId && it.period == period }
    }
    ...
    private fun createMockRepository(): Pair<MoneyPilotRepository, FakeDaos> {
        val categoryDao = FakeCategoryDao()
        val transactionDao = FakeTransactionDao()
        val budgetDao = FakeBudgetDao()
        val investmentDao = FakeInvestmentDao()
        ...
        whenever(loanDao.getAllLoans()).thenReturn(emptyFlow())

        val loanPaymentDao = mock(LoanPaymentDao::class.java)

        val pendingTransactionDao = mock(PendingTransactionDao::class.java)
        whenever(pendingTransactionDao.getAllPendingTransactions()).thenReturn(emptyFlow())

        val bookmarkedArticleDao = mock(BookmarkedArticleDao::class.java)
        whenever(bookmarkedArticleDao.getAllBookmarks()).thenReturn(emptyFlow())
        ...
        val fakes = FakeDaos(
            categoryDao, transactionDao, budgetDao, investmentDao, emergencyFundDao
        )
